...
- **Data dictionary** — every table, column, measure, and relationship, **generated from the model TMDL** (do not hand-edit; regenerate after any model change with `python tools/build_data_dictionary.py`): [docs/data-dictionary.md](docs/data-dictionary.md). Every model object carries a `///` description, so each field self-documents as a hover tooltip in Power BI. Current model: **35 tables · 360 columns · 162 measures · 47 relationships · 2 RLS roles** (the dictionary's provenance header stamps the model's git commit + the platform version).
- **Admin / operations map** — every authoring **List → sync process → DB table → bi view → model table → measures → audit**, with **live clickable SharePoint links** and the scheduled jobs + RLS roles, **generated** by `python tools/build_admin_map.py` (a coverage gate `--audit` fails if a List is added without mapping it): [docs/admin-map.md](docs/admin-map.md).
